...
(deftest test-is-allowed
  (is (r/is-allowed? board 3 0 1)))

(deftest test-is-allowed-not-allowed
  (is (not (r/is-allowed? board 2 0 1))))

(deftest test-row-horizontal
  (is (r/check-row-horizontal board 1 2 6)))

(deftest test-row-horizontal-not-allowed
  (is (not (r/check-row-horizontal board 2 0 1))))

(deftest test-row-vertical
  (is (r/check-row-vertical board 3 0 1)))

(deftest test-row-vertical-not-allowed
  (is (not (r/check-row-vertical board 2 0 1))))

(deftest test-square
  (is (r/check-square board 1 0 7)))

(deftest test-square-not-allowed
  (is (not (r/check-square board 7 0 1))))
...
(deftest test-square-3nd
  (is (r/check-square board 1 8 0)))

(deftest test-square-3nd-not-allowed
  (is (not (r/check-square board 4 0 8))))

(deftest test-get-allowed-values
  (is (= '(3 5 8) (r/get-allowed-values board 0 1))))

(deftest test-get-allowed-positions-horizontal
  (is (= '([0 1] [0 2] [0 3]) (r/get-allowed-positions-horizontal board 5 0 1))))

(deftest test-get-allowed-positions-vertical
  (is (= '([0 1] [1 1] [3 1] [4 1]) (r/get-allowed-positions-vertical board 5 0 1))))

(deftest test-get-allowed-positions-square
  (is (= '([0 1] [0 2] [1 1] [1 2]) (r/get-allowed-positions-square board 5 0 1))))

(deftest test-get-allowed-positions-horizontal-2nd
  (is (= '([2 6]) (r/get-allowed-positions-horizontal board 1 2 6))))

(deftest test-get-allowed-positions-vertical-2nd
  (is (= '([2 6] (r/get-allowed-positions-vertical board 1 2 6)))))

(deftest test-get-allowed-positions-square-2nd
  (is (= '([2 6]) (r/get-allowed-positions-square board 1 2 6))))

(deftest test-get-allowed-positions-horizontal-3nd
  (is (= '([2 2] [2 6]) (r/get-allowed-positions-horizontal board 9 2 6))))

(deftest test-get-allowed-positions-vertical-3nd
  (is (= '([2 6] [3 6] [5 6] [8 6]) (r/get-allowed-positions-vertical board 9 2 6))))

(deftest test-get-allowed-positions-square-3nd
  (is (= '([1 7] [2 6]) (r/get-allowed-positions-square board 9 2 6))))

(deftest test-only-possible-position
  (is (r/only-possible-position? board 1 2 6)))

(deftest test-only-possible-position-not
  (is (not (r/only-possible-position? board 5 1 0))))
